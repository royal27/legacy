...
if ($result) {
    while ($plugin = $result->fetch_assoc()) {
        ...
        $plugins_base_path = __DIR__ . DS . '..' . DS . 'plugins' . DS;

        $manifest_file = $plugins_base_path . $plugin_dir . DS . 'plugin.json';
        if (file_exists($manifest_file)) {
            $manifest = json_decode(file_get_contents($manifest_file), true);
            if (isset($manifest['main_file'])) {
                $main_plugin_file = $plugins_base_path . $plugin_dir . DS . $manifest['main_file'];
