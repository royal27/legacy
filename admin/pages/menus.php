...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'save_item') {
        ...
        if (!empty($title) && !empty($url)) {
            if ($id > 0) { // Update
                $stmt = $db->prepare("UPDATE menu_items SET title = ?, url = ?, target = ? WHERE id = ?");
                $stmt->bind_param('sssi', $title, $url, $target, $id);
                $message = 'Menu item updated.';
            } else { // Insert
                $stmt = $db->prepare("INSERT INTO menu_items (title, url, target, menu_location) VALUES (?, ?, ?, ?)");
                $stmt->bind_param('ssss', $title, $url, $target, $menu_location);
